...
object JarvisUnifiedAgent {
 ...
 fun stop(c:Context):String{val result=JarvisAgent.stop(c);c.getSharedPreferences(PREFS,0).edit().putString("state","stopped").putString("last_result",result).apply();return result}\n fun resume(c:Context):String{val result=JarvisAgent.resume(c);c.getSharedPreferences(PREFS,0).edit().putString("state","running").putString("last_result",result).apply();return result}\n fun explain(c:Context):String{val p=c.getSharedPreferences(PREFS,0);return p.getString("last_explanation","No unified agent action has run yet.").orEmpty()}
