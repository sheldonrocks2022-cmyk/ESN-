...
class JarvisVoiceService : Service(), TextToSpeech.OnInitListener {
    ...
    private fun voiceIdentity(text:String):String {\n        var s=text.trim().replace(Regex("\\s+")," ")\n        s=s.replace("Okay.", "Understood.").replace("OK.", "Understood.")\n        if(s.length>260)s=s.take(257).trimEnd()+ "..."\n        return s\n    }\n    private fun speak(text:String){if(text.isBlank())return;\n        val voiced=voiceIdentity(text)listening=false;try{recognizer?.cancel()}catch(_:Throwable){};lastSpokenText=voiced.lowercase(Locale.US).replace(Regex("[^a-z0-9 ]"),"").trim();if(!ttsReady){pendingSpeech=text;initTts();return};try{starting=false;checkpoint("SPEAK:${voiced.take(100)}");tts?.speak(voiced,TextToSpeech.QUEUE_FLUSH,null,"jarvis-${System.currentTimeMillis()}")}catch(t:Throwable){checkpoint("SPEAK_EXCEPTION:${t.javaClass.simpleName}:${t.message.orEmpty()}");if(active)handler.postDelayed({startRecognition()},900L)}}
