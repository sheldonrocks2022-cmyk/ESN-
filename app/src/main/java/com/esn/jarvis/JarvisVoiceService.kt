...
class JarvisVoiceService : Service(), TextToSpeech.OnInitListener {
    ...
    override fun onInit(status:Int){ checkpoint("TTS_INIT:$status"); if(status==TextToSpeech.SUCCESS){try{tts?.language=Locale.US;tts?.setSpeechRate(0.82f);tts?.setPitch(0.82f);ttsReady=true;checkpoint("TTS_READY");pendingSpeech?.let{pendingSpeech=null;speak(it)}}catch(t:Throwable){checkpoint("TTS_SETUP_EXCEPTION:${t.javaClass.simpleName}:${t.message.orEmpty()}")}} }
    private fun speak(text:String){if(text.isBlank())return;if(!ttsReady){pendingSpeech=text;initTts();return};try{starting=false;recognizer?.cancel();checkpoint("SPEAK:${text.take(100)}");tts?.speak(text,TextToSpeech.QUEUE_FLUSH,null,"jarvis-response");handler.postDelayed({if(active)startRecognition()},maxOf(1200L,text.length*55L))}catch(t:Throwable){checkpoint("SPEAK_EXCEPTION:${t.javaClass.simpleName}:${t.message.orEmpty()}");if(active)handler.postDelayed({startRecognition()},1200L)}}
