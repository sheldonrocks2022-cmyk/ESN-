...
class JarvisVoiceService : Service(), TextToSpeech.OnInitListener {
    ...
    override fun onInit(status:Int){checkpoint("TTS_INIT:$status");if(status==TextToSpeech.SUCCESS){try{configureVoice();tts?.setOnUtteranceProgressListener(object:UtteranceProgressListener(){override fun onStart(id:String){speaking=true;checkpoint("SPEAKING");handler.postDelayed({if(active)startRecognition()},150L)};override fun onDone(id:String){speaking=false;ttsRecoveryAttempts=0;checkpoint("SPEAK_DONE");handler.postDelayed({if(active)startRecognition()},250L)};@Deprecated("Deprecated in Java") override fun onError(id:String){speaking=false;ttsRecoveryAttempts++;checkpoint("SPEAK_ERROR:RECOVERY:$ttsRecoveryAttempts");try{tts?.shutdown()}catch(_:Throwable){};tts=null;ttsReady=false;if(ttsRecoveryAttempts<=3)handler.postDelayed({initTts()},500L*ttsRecoveryAttempts);handler.postDelayed({if(active)startRecognition()},700L)}});ttsReady=true;checkpoint("TTS_READY:${tts?.voice?.name.orEmpty()}");pendingSpeech?.let{pendingSpeech=null;speak(it)}}catch(t:Throwable){checkpoint("TTS_SETUP_EXCEPTION:${t.javaClass.simpleName}:${t.message.orEmpty()}")}}}
    private fun configureVoice(){val engine=tts?:return;engine.language=Locale.US;val prefs=getSharedPreferences(PREFS,MODE_PRIVATE);val selected=prefs.getString("tts_voice","").orEmpty();val preferred=engine.voices?.firstOrNull{it.name==selected&&!it.isNetworkConnectionRequired}?:chooseCalmEnglishVoice(engine.voices);if(preferred!=null){engine.voice=preferred;checkpoint("TTS_VOICE:${preferred.name}")};engine.setSpeechRate(prefs.getFloat("speech_rate",0.76f));engine.setPitch(prefs.getFloat("speech_pitch",0.68f))}
    ...
    private fun watchdog(){if(!active)return;val stale=lastHealthyAt>0&&System.currentTimeMillis()-lastHealthyAt>20000;if(stale&&!starting){checkpoint("WATCHDOG_RECOVERY");try{recognizer?.cancel();recognizer?.destroy()}catch(_:Throwable){};recognizer=null;starting=false;startRecognition()};handler.postDelayed({watchdog()},10000L)}
