...
class JarvisVoiceService : Service(), TextToSpeech.OnInitListener {
    ...
    override fun onInit(status:Int){checkpoint("TTS_INIT:$status");if(status==TextToSpeech.SUCCESS){try{configureVoice();tts?.setOnUtteranceProgressListener(object:UtteranceProgressListener(){override fun onStart(id:String){speaking=true;checkpoint("SPEAKING");handler.postDelayed({if(active)startRecognition()},150L)};override fun onDone(id:String){speaking=false;lastTtsFinishedAt=System.currentTimeMillis();ttsRecoveryAttempts=0;checkpoint("SPEAK_DONE");handler.postDelayed({if(active)startRecognition()},250L)};@Deprecated("Deprecated in Java") override fun onError(id:String){speaking=false;lastTtsFinishedAt=System.currentTimeMillis();ttsRecoveryAttempts++;checkpoint("SPEAK_ERROR:RECOVERY:$ttsRecoveryAttempts");try{tts?.shutdown()}catch(t:Throwable){JarvisDiagnostics.recordFailure(this@JarvisVoiceService,"voice_recovery",t.javaClass.simpleName+":"+t.message.orEmpty())};tts=null;ttsReady=false;if(ttsRecoveryAttempts<=3)handler.postDelayed({initTts()},500L*ttsRecoveryAttempts);handler.postDelayed({if(active)startRecognition()},700L)}});ttsReady=true;checkpoint("TTS_READY:${tts?.voice?.name.orEmpty()}");pendingSpeech?.let{pendingSpeech=null;speak(it)}}catch(t:Throwable){checkpoint("TTS_SETUP_EXCEPTION:${t.javaClass.simpleName}:${t.message.orEmpty()}")}}}
    private fun configureVoice(){val engine=tts?:return;engine.language=Locale.US;val prefs=getSharedPreferences(PREFS,MODE_PRIVATE);val selected=prefs.getString("tts_voice","").orEmpty();val gender="male";prefs.edit().putString("voice_gender","male").apply();val selectedVoice=engine.voices?.firstOrNull{it.name==selected&&!it.isNetworkConnectionRequired&&!looksFemale(it)};val preferred=selectedVoice?:chooseGenderVoice(engine.voices,gender)?:chooseCalmEnglishVoice(engine.voices);if(preferred!=null){engine.voice=preferred;checkpoint("TTS_VOICE:${preferred.name}")};engine.setSpeechRate(prefs.getFloat("speech_rate",0.72f));engine.setPitch(prefs.getFloat("speech_pitch",0.64f))}
    private fun looksFemale(v:Voice):Boolean{val n=v.name.lowercase(Locale.US);return listOf("female","feminine","woman").any{n.contains(it)}}
    private fun chooseGenderVoice(voices:Set<Voice>?,gender:String):Voice?{if(voices.isNullOrEmpty())return null;val english=voices.filter{it.locale?.language==Locale.ENGLISH.language&&!it.isNetworkConnectionRequired&&!looksFemale(it)};if(english.isEmpty())return null;val keys=listOf("male","masculine","man","low","deep");val matched=english.filter{v->keys.any{v.name.lowercase(Locale.US).contains(it)}};return matched.maxByOrNull{if(it.locale==Locale.US)20 else 0}}
    private fun chooseCalmEnglishVoice(voices:Set<Voice>?):Voice?{if(voices.isNullOrEmpty())return null;val english=voices.filter{it.locale?.language==Locale.ENGLISH.language&&!it.isNetworkConnectionRequired&&!looksFemale(it)};if(english.isEmpty())return null;fun score(v:Voice):Int{val n=v.name.lowercase(Locale.US);var s=0;if(v.locale==Locale.US)s+=30;if(!v.isNetworkConnectionRequired)s+=20;if(n.contains("male")||n.contains("masculine"))s+=120;if(n.contains("low")||n.contains("deep"))s+=35;if(n.contains("en-us"))s+=15;if(n.contains("local"))s+=10;return s};return english.maxByOrNull(::score)}
    private fun voiceIdentity(text:String):String {
        var s=text.trim().replace(Regex("\\s+")," ")
        s=s.replace("Okay.", "Understood.").replace("OK.", "Understood.")
        if(s.length>260)s=s.take(257).trimEnd()+"..."
        return s
    }
    private fun speak(text:String){
        if(text.isBlank())return
        val voiced=voiceIdentity(JarvisPersonality.voiceLine(this,text))
        listening=false
        try{recognizer?.cancel()}catch(t:Throwable){JarvisDiagnostics.recordFailure(this@JarvisVoiceService,"voice_cleanup",t.javaClass.simpleName+":"+t.message.orEmpty())}
        lastSpokenText=voiced.lowercase(Locale.US).replace(Regex("[^a-z0-9 ]"),"").trim()
        if(!ttsReady){pendingSpeech=voiced;initTts();return}
        try{
            starting=false
            checkpoint("SPEAK:${voiced.take(100)}")
            tts?.speak(voiced,TextToSpeech.QUEUE_FLUSH,null,"jarvis-${System.currentTimeMillis()}")
        }catch(t:Throwable){
            checkpoint("SPEAK_EXCEPTION:${t.javaClass.simpleName}:${t.message.orEmpty()}")
            if(active)handler.postDelayed({startRecognition()},900L)
        }
    }
    ...
    private fun handleSpeech(spoken:String){
        ...
        if(normalized=="jarvis code 101"||normalized=="jarvis code one oh one"||normalized=="jarvis code one zero one"){
            ...
            // Standby keeps wake-word recognition alive so "JARVIS start up" can wake the assistant.
        ...
        if(command in setOf("stop listening","end conversation","cancel conversation")){pendingConfirmation=null;conversationUntil=0L;JarvisUnifiedAgent.stop(this);speak("Conversation ended.");return}
        if(command in setOf("wait","hold on")){JarvisUnifiedAgent.stop(this);conversationUntil=System.currentTimeMillis()+conversationWindowMs;speak("Standing by.");return}
        if(command.startsWith("actually ")){JarvisUnifiedAgent.stop(this);conversationUntil=System.currentTimeMillis()+conversationWindowMs;handleSpeech("jarvis "+command.removePrefix("actually "));return}
    ...
    private fun watchdog(){if(!active)return;val stale=listeningSince==0L&&lastHealthyAt>0&&System.currentTimeMillis()-lastHealthyAt>20000;if(stale&&!starting){checkpoint("WATCHDOG_RECOVERY");try{recognizer?.cancel();recognizer?.destroy()}catch(t:Throwable){JarvisDiagnostics.recordFailure(this@JarvisVoiceService,"voice_cleanup",t.javaClass.simpleName+":"+t.message.orEmpty())};recognizer=null;starting=false;startRecognition()};handler.postDelayed({watchdog()},10000L)}
    private fun deactivate(){active=false;starting=false;listening=false;handler.removeCallbacksAndMessages(null);try{recognizer?.cancel();recognizer?.destroy()}catch(t:Throwable){JarvisDiagnostics.recordFailure(this@JarvisVoiceService,"voice_cleanup",t.javaClass.simpleName+":"+t.message.orEmpty())};recognizer=null;try{tts?.stop();tts?.shutdown()}catch(t:Throwable){JarvisDiagnostics.recordFailure(this@JarvisVoiceService,"voice_cleanup",t.javaClass.simpleName+":"+t.message.orEmpty())};tts=null;ttsReady=false;setActive(false);checkpoint("STOP_COMPLETE");try{ServiceCompat.stopForeground(this,ServiceCompat.STOP_FOREGROUND_REMOVE)}catch(t:Throwable){JarvisDiagnostics.recordFailure(this@JarvisVoiceService,"voice_cleanup",t.javaClass.simpleName+":"+t.message.orEmpty())};stopSelf()}
    ...
    override fun onDestroy(){handler.removeCallbacksAndMessages(null);listening=false;try{recognizer?.cancel();recognizer?.destroy()}catch(t:Throwable){JarvisDiagnostics.recordFailure(this@JarvisVoiceService,"voice_cleanup",t.javaClass.simpleName+":"+t.message.orEmpty())};recognizer=null;try{tts?.stop();tts?.shutdown()}catch(t:Throwable){JarvisDiagnostics.recordFailure(this@JarvisVoiceService,"voice_cleanup",t.javaClass.simpleName+":"+t.message.orEmpty())};tts=null;super.onDestroy()}
