...
class JarvisVoiceService : Service(), TextToSpeech.OnInitListener {
    ...
    override fun onInit(status:Int){checkpoint("TTS_INIT:$status");if(status==TextToSpeech.SUCCESS){try{configureVoice();tts?.setOnUtteranceProgressListener(object:UtteranceProgressListener(){override fun onStart(id:String){speaking=true;checkpoint("SPEAKING");handler.postDelayed({if(active)startRecognition()},150L)};override fun onDone(id:String){speaking=false;checkpoint("SPEAK_DONE");handler.postDelayed({if(active)startRecognition()},250L)};@Deprecated("Deprecated in Java") override fun onError(id:String){speaking=false;checkpoint("SPEAK_ERROR");handler.postDelayed({if(active)startRecognition()},700L)}});ttsReady=true;checkpoint("TTS_READY:${tts?.voice?.name.orEmpty()}");pendingSpeech?.let{pendingSpeech=null;speak(it)}}catch(t:Throwable){checkpoint("TTS_SETUP_EXCEPTION:${t.javaClass.simpleName}:${t.message.orEmpty()}")}}}
    ...
    private fun handleSpeech(spoken:String){
        ...
        val command=normalized.replaceFirst(Regex("^(jarvis|jervis|jarvises|service|drivers)\\s+"),"").trim()
