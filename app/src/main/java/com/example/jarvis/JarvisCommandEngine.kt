...
class JarvisCommandEngine(private val context: Context) {
    fun execute(command: String): String {
        ...
        if (c == "turn on wifi" || c == "turn off wifi") return openSettingsPanel(Settings.ACTION_WIFI_SETTINGS, "Wi-Fi")
        ...
        if (c == "open music") return launchPackage("com.spotify.music", "Spotify")

        if (c == "jarvis help" || c == "help" || c == "what can you do") return "I can text, call, open apps, search the web, control supported audio and flashlight functions, open Wi-Fi and Bluetooth settings, set timers and alarms, tell the time and date, and use Phone Access for supported screen actions."
    ...
    } catch (_: Exception) { "I couldn't open the messaging app for that text." }
    ...
    private fun launchPackage(packageName: String, appName: String): String {
        return try {
            ...
            if (intent == null) return "$appName isn't installed on this phone."
    ...
    private fun setTimer(value: String): String {
        return try {
            ...
            val intent = Intent(AlarmClock.ACTION_SET_TIMER).apply { putExtra(AlarmClock.EXTRA_LENGTH, seconds); putExtra(AlarmClock.EXTRA_SKIP_UI, false); addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) }
            context.startActivity(intent)
    ...
    private fun setAlarm(value: String): String {
        return try {
            ...
            val intent = Intent(AlarmClock.ACTION_SET_ALARM).apply { putExtra(AlarmClock.EXTRA_HOUR, hour); putExtra(AlarmClock.EXTRA_MINUTES, minute); putExtra(AlarmClock.EXTRA_SKIP_UI, false); addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) }
            context.startActivity(intent)
    ...
    private fun parseDurationSeconds(value: String): Int {
        ...
        return when { text.contains("hour") -> number * 3600; text.contains("minute") || text.contains("min") -> number * 60; else -> number }
    }
